...
public class PactTranslationBridge extends RouteBuilder {
    ...
    public void configure() throws Exception {
        from("kafka:pact-untranslated?brokers=pact-cluster-kafka-brokers.pact.svc.cluster.local:9092").process(new LocProcessor())
        .log("uri ${body}")
        .toD("http://${body}").process(new MapRespProcessor())
        .log("body consumed ${body}")    
        .removeHeaders("*")
        .to("kafka:pact-translated?brokers=pact-cluster-kafka-brokers.pact.svc.cluster.local:9092");
